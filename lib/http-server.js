...
function processRequest(req, res) {
	try {
		cservice.options.log("cluster-service::REST: " + req.url);
		...
		cservice.options.log("cluster-service: Woops, an ERROR!", util.inspect(ex, {depth:null}), util.inspect(ex.stack || new Error().stack, {depth:null}));
	}
}
...
function onCommand(req, res, question) {
	...
	if (!locals.events[args[0]]) {
		cservice.options.log("Command " + args[0] + " not found");
		res.writeHead(404);
