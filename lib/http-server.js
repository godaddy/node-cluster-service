...
function processRequest(req, res) {
	try {
		cservice.log("cluster-service::REST: " + req.url);
		...
		cservice.log("cluster-service: Woops, an ERROR!", util.inspect(ex, {depth:null}), util.inspect(ex.stack || new Error().stack, {depth:null}));
	}
}
...
function onCommand(req, res, question) {
	...
	if (!locals.events[args[0]]) {
		cservice.log("Command " + args[0] + " not found");
		res.writeHead(404);
		...
		if (err) { // should do nothing if response already sent
			...
			if (result) {
				...
				try
				{
					...
					cservice.log("cluster-service: Woops, an ERROR!", err, util.inspect(ex.stack || new Error().stack, {depth:null}));
				}
			} else {
				res.writeHead(200);
				res.end("No data");
