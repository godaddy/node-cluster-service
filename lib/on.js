...
if (cluster.isMaster === true && cservice.locals.firstTime === true) {
	...
	on("workerStart", function(evt, worker, reason) {
		cservice.log(("Worker " + path.basename(worker.cservice.worker) + "(" + worker.process.pid + ") start, reason: " + (reason || cservice.locals.reason || "Unknown")).success);
	}, false);
	on("workerExit", function(evt, worker, reason) {
		cservice.log(("Worker " + path.basename(worker.cservice.worker) + "(" + worker.process.pid + ") exited, reason: " + (reason || cservice.locals.reason || "Unknown")).warn);
	}, false);
}
