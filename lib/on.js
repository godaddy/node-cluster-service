...
if (cluster.isMaster === true && cservice.locals.firstTime === true) {
	...
	on("workerStart", function(evt, pid, reason) {
		cservice.log(("Worker " + pid + " start, reason: " + (reason || cservice.locals.reason || "Unknown")).success);
	}, false);
	on("workerExit", function(evt, pid, reason) {
		cservice.log(("Worker " + pid + " exited, reason: " + (reason || cservice.locals.reason || "Unknown")).warn);
	}, false);
}
