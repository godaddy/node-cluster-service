...
function startMaster(options, cb) {
	...
	if (cservice.locals.state === 0) { // one-time initializers
		...
		/*process.on("uncaughtException", function(err) {
			cservice.options.log("uncaughtException", util.inspect(err));
		});*/
...
function startListener(options, cb) {
	if (typeof options.accessKey !== "string") { // in-proc mode only
		cservice.options.log("cluster-service is in LOCAL ONLY MODE. Run with 'accessKey' option to enable communication channel.");
		cb();
		return;
	}
	...
	httpserver.init(cservice.locals, options, function(err) {
		if (!err) {
			cservice.options.log("cluster-service is listening at " + options.host + ":" + options.port);
		}
