...
function startListener(options, cb) {
	if (typeof options.accessKey !== "string") { // in-proc mode only
		cservice.log("cluster-service is in LOCAL ONLY MODE. Run with 'accessKey' option to enable communication channel.");
		cb();
		return;
	}
	...
	httpserver.init(cservice.locals, options, function(err) {
		if (!err) {
			cservice.log("cluster-service is listening at " + options.host + ":" + options.port);
		}
